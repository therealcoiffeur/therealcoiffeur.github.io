...
Certainly the identification of these vulnerabilities may seem trivial, but, what will be interesting now is to identify the presence of these vulnerabilities in an automated way, thanks to a static analysis tool that we will try to develop in the next bonus chapter.
{% endraw %}