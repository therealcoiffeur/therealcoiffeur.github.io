...
The first reflex is to identify which part of the source code can be reached as an unauthenticated user. To do this I use my browser and with the help of [Burp Suite](https://portswigger.net/burp), I look at which scripts are called and with which parameters these are called.
...
First, I realize that the script <span style="color:red">/usr/share/zabbix/jsLoader.php</span> takes in parameter the variables:
...
File:<span style="color:red">\<ROOT\>/nclude/defines.inc.php</span> (from v5.0.3) <=> <span style="color:red">\<ROOT\>/ui/include/defines.inc.php</span> (from commit 175b839dc6185f572f95ff87b8728218bed54341)
```php
...
The vulnerability having been identified, we must now try to find gadgets allowing us to carry out an attack. In case I discover one of these gadgets, I will publish a new article.
